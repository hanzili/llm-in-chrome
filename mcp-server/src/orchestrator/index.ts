...
import { planningAgent, type PlanningOutput } from "../agents/planning.js";
 ...
 */
export type BrowserExecuteCallback = (
  sessionId: string,
  task: string,
  url?: string,
  context?: string,
...
export class Orchestrator {
  ...
  private browserExecute: BrowserExecuteCallback | null = null;
   ...
   */
  async startTask(options: CreateSessionOptions): Promise<StartTaskResult> {
    // Create the session
    ...
    if (planningResult.planningTrace && planningResult.planningTrace.length > 0) {
      for (const traceStep of planningResult.planningTrace) {
        ...
        let traceType: "planning_agent:search_knowledge" | "planning_agent:read_knowledge" | "planning_agent:query_memory" | "planning_agent:list_domains" | "planning_agent:finish" | "planning_agent:info" | "planning_agent:error" = "planning_agent:info";
        if (traceStep.includes("search_knowledge")) traceType = "planning_agent:search_knowledge";
        else if (traceStep.includes("read_knowledge")) traceType = "planning_agent:read_knowledge";
        else if (traceStep.includes("query_memory")) traceType = "planning_agent:query_memory";
        else if (traceStep.includes("list_domains")) traceType = "planning_agent:list_domains";
        else if (traceStep.includes("finish_planning") || traceStep.includes("Planning complete")) traceType = "planning_agent:finish";
        else if (traceStep.includes("error") || traceStep.includes("Error")) traceType = "planning_agent:error";
    ...
    if (exploration && exploration.type !== 'none' && this.browserExecute && planningResult.domain) {
      ...
      if (explorationType === 'overview') {
        explorationTask = explorerAgent.createOverviewTask(
          domain,
          options.url || `https://${domain}`
        );
      } else {
        // workflow exploration
        const workflowTask = 'task' in exploration ? exploration.task : options.task;
        explorationTask = explorerAgent.createWorkflowTask(
          domain,
          options.url || `https://${domain}`,
          workflowTask
        );
      ...
      await this.browserExecute(
        sessionId,
        explorationTask.explorationPrompt,
        explorationTask.url,
        explorationType === 'overview'
          ? "Quick overview only - just tell me what this site is in 2-3 sentences."
          : "Explore this workflow - don't complete it, just report the steps.",
        undefined // No site knowledge yet
      );
    ...
    if (!planningResult.readyToExecute) {
      ...
      this.sessionManager.setPendingQuestions(
        sessionId,
        planningResult.missingInfo.map((info, i) => ({
          id: `q-${i}`,
          field: info.field,
    ...
    if (this.browserExecute) {
      ...
      await this.browserExecute(
        ...
        planningResult.siteKnowledge?.content
      );
    }
   ...
   */
  async processExplorationComplete(
    sessionId: string,
    ...
    await explorerAgent.processExplorationReport(
      session.domain,
      explorationReport,
      session.collectedInfo._originalTask
    );
  }
   ...
   */
  async continueAfterExploration(sessionId: string): Promise<void> {
    const session = this.sessionManager.getSession(sessionId);
    if (!session) {
    ...
    if (session.state === "NEEDS_INFO") {
      ...
      if (this.browserExecute) {
        await this.browserExecute(
          ...
          updatedSession.url,
          fullContext,
          planningResult.siteKnowledge?.content || updatedSession.siteKnowledge?.content
        );
      }
   ...
   */
  private parseUserResponse(message: string): Record<string, string> {
   ...
   */
  private formatCollectedInfo(info: Record<string, string>): string {
   ...
   */
  updateFromBrowserEvent(
    sessionId: string,
    eventType: "progress" | "complete" | "error" | "blocked",
    data: {
      step?: string;
      answer?: string;
      error?: string;
    ...
    switch (eventType) {
      ...
      case "complete":
        ...
        if (wasExploring && completingSession?.domain) {
          ...
          this.processExplorationComplete(
            sessionId,
            data.answer || completingSession.answer || ""
          ).then(() => {
