...
import { askLLM, askLLMForJSON, isLLMAvailable } from "./llm/client.js";
...
interface SessionMemory {
  sessionId: string;
  facts: string[];           // Extracted/added facts
  rawContext: string;        // Original raw context (fallback)
  createdAt: string;
  updatedAt: string;
}
...
interface SearchResult {
  memory: string;
  score: number;             // 1.0 = highly relevant, 0.0 = not relevant
}
 ...
 */
export async function addFacts(sessionId: string, facts: string[]): Promise<void> {
 ...
 */
export function getRawContext(sessionId: string): string | null {
...
export function getMemoryStats(): {
  sessionCount: number;
  totalFacts: number;
  sessions: { id: string; factCount: number; hasRawContext: boolean }[];
} {
 ...
 */
async function extractFacts(context: string): Promise<string[]> {
  // Skip LLM for very short context
  if (context.length < 50) {
    return [context.trim()];
  }

  const response = await askLLMForJSON<{ facts: string[] }>({
 ...
 */
async function findRelevantFacts(
  facts: string[],
  query: string,
  limit: number
): Promise<SearchResult[]> {
 ...
 */
async function rankFactsWithLLM(
  facts: string[],
  query: string,
  limit: number
): Promise<SearchResult[]> {
  ...
  const response = await askLLMForJSON<{ relevant: { index: number; score: number }[] }>({
