...
export interface ExplorationTask {
  ...
  workflowName?: string;  // For workflow explorations
}
...
export class ExplorerAgent {
   ...
   */
  async needsOverviewExploration(domain: string): Promise<boolean> {
   ...
   */
  async findWorkflow(domain: string, taskDescription: string): Promise<string | null> {
    const knowledge = await getKnowledge(domain);
...
If NO workflow exists for this task, respond with just: NO_WORKFLOW`,
   ...
   */
  async needsExploration(domain: string): Promise<boolean> {
...
`.trim();
   ...
   */
  private generateWorkflowName(taskDescription: string): string {
   ...
   */
  async processOverviewReport(domain: string, report: string): Promise<ExplorationResult> {
   ...
   */
  async processExplorationReport(
    domain: string,
    report: string,
    taskHint?: string
  ): Promise<ExplorationResult> {
   ...
   */
  private async writeOverviewMarkdown(domain: string, report: string): Promise<string> {
    const response = await askLLM({
...
Keep it SHORT. No extra details.`,
   ...
   */
  private async writeWorkflowMarkdown(
    workflowName: string,
    taskDescription: string,
    report: string
  ): Promise<string> {
