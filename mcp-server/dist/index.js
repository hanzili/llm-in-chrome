...
import { NativeHostConnection } from "./ipc/index.js";
// Memory layer (Mem0)
import { initializeMemory, storeContext, searchMemory, deleteSessionMemories, isMemoryAvailable, getRawContext, } from "./memory-lite.js";
 ...
 */
function getBrowserExtras(sessionId) {
 ...
 */
function mapStateToLegacyStatus(state) {
    const mapping = {
        CREATED: "starting",
        PLANNING: "running",
...
async function handleNativeMessage(message) {
    ...
    if (type === 'mcp_results' && Array.isArray(results)) {
        if (DEBUG_IPC)
            console.error(`[MCP] Processing ${results.length} results from poll`);
...
function processResult(result) {
    ...
    if (DEBUG_IPC)
        console.error(`[MCP] processResult: type=${type}, sessionId=${sessionId}`);
 ...
 */
function mapStateToAgentPhase(state, session) {
    if (state === 'PLANNING')
        return 'planning_agent';
    if (state === 'EXECUTING' && session?.collectedInfo?._exploring === 'true')
...
function formatSession(sessionId) {
    ...
    if (session?.executionTrace && session.executionTrace.length > 0) {
        ...
        const planningSteps = session.executionTrace
            .filter((t) => t.type?.startsWith('planning_agent:'))
            .map((t) => ({ type: t.type, description: t.description, timestamp: t.timestamp }))
            .slice(-TRACE_LIMIT);
        ...
        const explorerSteps = session.executionTrace
            .filter((t) => t.type?.startsWith('explorer_agent:'))
            .map((t) => ({ type: t.type, description: t.description, timestamp: t.timestamp }))
            .slice(-TRACE_LIMIT);
        ...
        const browserSteps = session.executionTrace
            .filter((t) => t.type?.startsWith('browser_agent:') || !t.type?.includes('_agent:'));
        if (browserSteps.length > 0) {
            response.browser_agent_actions = browserSteps.length;
            // Only show last few for context
            response.recent_browser_actions = browserSteps.slice(-5).map((t) => t.description);
        }
    }
        ...
        switch (name) {
            case "browser_start": {
                ...
                const planningAgentTrace = session?.executionTrace
                    ?.filter((t) => t.type?.startsWith('planning_agent:'))
                    .map((t) => ({ type: t.type, description: t.description })) || [];
            ...
            case "browser_stop": {
                ...
                if (shouldRemove) {
                    ...
                    if (isMemoryAvailable()) {
                        deleteSessionMemories(sessionId).catch(err => console.error(`[MCP] Failed to clean up memories for ${sessionId}:`, err));
                    }
                    return {
                        content: [{
                                type: "text",
